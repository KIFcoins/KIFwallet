...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("KIF://"))
    {
        uri.replace(0, 12, "KIF:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "KIF.lnk";
}
...
{
    return GetAutostartDir() / "KIF.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("KIF-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  KIF-qt [" + tr("command-line options") + "]                     " + "\n";
